...
import { ILigneCommande, LigneCommande } from '../model/ligne-commande.model';
...
export class CommandeUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.commandeService.getCommande(localStorage.getItem("idcommande")).subscribe(
      (res) => {
        this.commande = res
        this.editForm = this.commande
        this.commande.lignescommandes.forEach(element => {
          this.ligneCommande.push(element)
        });
        this.selectclient = this.commande.client.id
      },
      (err) => {
        console.log(err);
      }
  ...
  save() {
    ...
    this.facture.id = this.editForm.facture.id
    this.facture.date = formatDate(new Date(), 'MM/dd/yyyy', 'en');
  ...
  filtrerProduits() {
    this.produits = this.produitss.filter((item) =>
      item.reference.includes(this.filtreproduit)
    );
  }
  ...
  generatePdf() {
      ...
      content: [
        ...
        },
        {
          columns: [
            ...
            {
              text: this.editForm.client.adresse,
              style: 'invoiceBillingAddress',
            },
          ],
        },
          ...
          table: {
            ...
            body: [
              [
                ...
                {
                  border: [false, true, false, true],
                  text: this.formatNumber(this.facture.total),
                  alignment: 'right',
                  fillColor: '#f5f5f5',
                  margin: [0, 5, 0, 5],
                },
              ...
              [
                ...
                {
                  text: this.formatNumber(this.facture.tva),
                  border: [false, false, false, true],
                  fillColor: '#f5f5f5',
                  alignment: 'right',
                  margin: [0, 5, 0, 5],
    ...
    //   this.myFile = utf8.buffer; // Convert to Binary...
    // });
    ...
    pdfDocGenerator.getBase64((blob) => {
      ...
      this.factureService.addFacture(this.facture).subscribe(
        (res) => {
          ...
          this.commandeService.editCommande(this.editForm).subscribe(
            (res) => {
              this.router.navigate(['commande']);
            },
            (err) => {
